...
public class VisualPiece : NetworkBehaviour {
	...
	public void OnMouseUp() {
		if (enabled) {
            ...
            SendMoveToServerRpc(closestSquareTransform.position);
        }
	}

    [ServerRpc(RequireOwnership = false)]
    void SendMoveToServerRpc(Vector3 newPosition)
    {
        UpdateMoveClientRpc(newPosition);
    }


    [ClientRpc]
    void UpdateMoveClientRpc(Vector3 newPosition)
    {
        // Don't re-apply the position on the one who moved it
        if (IsOwner) return;
