...
public class StoreItemsSO: ScriptableObject
{
    ...
    [ShowInInspector]
    [DictionaryDrawerSettings(KeyLabel = "Item ID", ValueLabel = "Thumbnail", DisplayMode = DictionaryDisplayOptions.ExpandedFoldout)]
    ...
    public void OnStoreItemListChanged()
    {
        Debug.Log("StoreItem");
    ...
    private void SimulateShopItems()
    {
        ...
        foreach (StoreItem item in Items)
        {
            ...
            if (Thumbnails.TryGetValue(item.ID, out var sprite))
            {
                Debug.Log($"🖼️ ID: {item.ID} has thumbnail: {(sprite != null ? sprite.name : "null")}");
            }
            else
            {
                Debug.LogWarning($"⚠️ ID {item.ID} is missing from Thumbnails dictionary.");
            }

            //1 sprite, 3 price, 4 name
            newStoreitem.transform.GetChild(0).GetComponent<Image>().sprite = item.ThumbnailImage;
            newStoreitem.transform.GetChild(2).GetComponent<TMP_Text>().text = item.Price.ToString();
            newStoreitem.transform.GetChild(1).GetComponent<TMP_Text>().text = item.Name;
        }
    }
